...
//Delay times
#define TIMEOUT_RFD_CTS_HL         3000
...
#define SAME 0
  ...
  iStr_len = strlen(cCmnd_buff);
  if(iStr_len > uiSend_frame_len)
  {
    uiSend_frame_len = iStr_len;
  }
...
#define DEBUG_iIneedmd_radio_rcv_settings
  ...
  eEC = eSend_Radio_CMND(SET_BT_GET, strlen(SET_BT_GET));
  if(eEC == ER_OK)
  {
    eEC = eIneedmd_radio_rcv_string(cRdo_setup_rcv_buff, BG_SIZE, &uiRcvd_len);
    if(eEC == ER_TIMEOUT)
    {
      eEC = ER_FAIL;
    }
    ...
    {
      iCmp_rslt = strcmp(cRdo_setup_rcv_buff, SET_BT_GET);
      if(iCmp_rslt == 0)
      {
        //echo is on, device not set properly
        //
        eEC = ER_FAIL;
      }
      else
  ...
  if(eEC == ER_OK)
  {
    if(uiRcvd_len == SET_BT_BDADDR_RCV_LEN)
    {
      cSetting = strstr(cRdo_setup_rcv_buff, SET_BT_BDADDR_CHK);
      if(cSetting == NULL)
      {
        eEC = ER_FAIL;
      }
      ...
      {
        eEC = ER_OK;
        iIneedmd_parse_addr(cRdo_setup_rcv_buff, uiBT_addr);
      }
    }
    else
    {
  ...
  if(eEC == ER_OK)
  {
    memset(cRdo_setup_rcv_buff, 0x00, strlen(cRdo_setup_rcv_buff));
    eEC = eIneedmd_radio_rcv_string(cRdo_setup_rcv_buff, BG_SIZE, &uiRcvd_len);
    if(eEC == ER_TIMEOUT)
    {
      eEC = ER_FAIL;
    ...
    {
      if(uiRcvd_len == SET_BT_NAME_RCV_LEN)
      {
        snprintf(cBT_setting, 64, SET_BT_NAME, uiBT_addr[4], uiBT_addr[5]);
        cSetting = strstr(cRdo_setup_rcv_buff, cBT_setting);
        if(cSetting == NULL)
        {
          eEC = ER_FAIL;
        }
  ...
  if(eEC == ER_OK)
  {
    ...
    while(eEC == ER_FAIL)
    {
      ...
      if(uiRcvd_len == SET_BT_AUTH_RCV_LEN)
      {
      ...
      else if(uiRcvd_len == ENDOF_SET_SETTINGS_STRLEN)
      {
        cSetting = strstr(cRdo_setup_rcv_buff, ENDOF_SET_SETTINGS);
        if(cSetting == NULL)
        {
          eEC = ER_OK;
        }
  ...
  if(eEC == ER_OK)
  {
    ...
    while(eEC == ER_FAIL)
    {
      ...
      if(uiRcvd_len == SET_BT_SSP_RCV_LEN)
      {
  ...
  if(eEC == ER_FAIL)
  {
    ...
    //
    //continue to recieve all the settings until the end is reached to clear the uart of traffic
    iCmp_rslt = -1;
    while(iCmp_rslt != 0)
    {
      memset(cRdo_setup_rcv_buff, 0x00, strlen(cRdo_setup_rcv_buff));
      ...
      iCmp_rslt = strcmp(cRdo_setup_rcv_buff, ENDOF_SET_SETTINGS);
    }
    eEC_set_bt = ER_FAIL;
  }
  else
  {
    ...
    //
    //continue until the end of the SET BT settings
    eEC = ER_FAIL;
    while(eEC == ER_FAIL)
    {
      memset(cRdo_setup_rcv_buff, 0x00, strlen(cRdo_setup_rcv_buff));
      ...
      if(uiRcvd_len == ENDOF_SET_SETTINGS_STRLEN)
      {
        cSetting = strstr(cRdo_setup_rcv_buff, ENDOF_SET_SETTINGS);
        if(cSetting == NULL)
        {
  ...
  if(eEC_set_bt == ER_OK)
  {
    ...
    //
    //begin set control settings check
    memset(cRdo_setup_rcv_buff, 0x00, BG_SIZE);
    eEC = eSend_Radio_CMND(SET_CONTROL_GET, strlen(SET_CONTROL_GET));
    ...
    if(eEC == ER_OK)
    {
      ...
      while(eEC == ER_FAIL)
      {
        ...
        if(uiRcvd_len == SET_CONTROL_BIND_RCV_LEN)
        {
    ...
    if(eEC == ER_OK)
    {
      ...
      while(eEC == ER_FAIL)
      {
        ...
        if(uiRcvd_len == SET_CONTROL_CONFIG_RCV_LEN)
        {
    ...
    if(eEC == ER_OK)
    {
      ...
      {
        memset(cRdo_setup_rcv_buff, 0x00, strlen(cRdo_setup_rcv_buff));
        eIneedmd_radio_rcv_string(cRdo_setup_rcv_buff, BG_SIZE, &uiRcvd_len);
        if(uiRcvd_len == SET_CONTROL_ESCAPE_RCV_LEN)
        {
          memset(cBT_setting, 0x00, 64);
    ...
    if(eEC == ER_OK)
    {
      ...
      while(eEC == ER_FAIL)
      {
        ...
        if(uiRcvd_len == SET_CONTROL_VREGEN_RCV_LEN)
        {
          memset(cBT_setting, 0x00, 64);
    ...
    if(eEC == ER_OK)
    {
      ...
      while(eEC == ER_FAIL)
      {
        ...
        if(uiRcvd_len == SET_CONTROL_BATT_RCV_LEN)
        {
          memset(cBT_setting, 0x00, 64);
    ...
    else
    {
      ...
      //
      //continue until the end of the SET CONTROL settings
      eEC = ER_FAIL;
      while(eEC == ER_FAIL)
      {
        memset(cRdo_setup_rcv_buff, 0x00, strlen(cRdo_setup_rcv_buff));
  ...
  //check if the SET BT and SET CONTROL settings all passed
...
//void vIneedMD_radio_read_cb(RADIO_INTERFACE_RXCB_PARAMS)
{
  ...
  if(count == 3)
  {
    ...
    if(c[0] == INEEDMD_CMND_SOF)
    {
      //Protocol frame
      //
      //recieve the rest of the protocol frame
      //Get the protocol frame len
      tParams.uiBuff_size = c[2];
...
//    if(eOSAL_Mailbox_Get_msg(&tRadio_mailbox) == ER_OK)
    {
      ...
      switch(tRadio_msg.eRequest)
      {
        ...
        case RADIO_REQUEST_POWER_OFF:
          break;
        case RADIO_REQUEST_SEND_FRAME:
          vDEBUG_RDIO_TASK("vIneedMD_radio_task, send frame");
          eEC = eIneedmd_radio_send_frame(tRadio_msg.uiBuff, tRadio_msg.uiBuff_size);
          if(tRadio_msg.vBuff_sent_callback != NULL)
          {
            if(eEC == ER_OK)
            {
              tRadio_msg.vBuff_sent_callback(tRadio_msg.uiBuff_size);
            }
            else
            {
              tRadio_msg.vBuff_sent_callback(0);
            }
          }
          break;
        case RADIO_REQUEST_RECEIVE_RADIO_FRAME:
        ...
        case RADIO_REQUEST_RECEIVE_PROTOCOL_FRAME:
          ...
          if((tRadio_msg.uiBuff[0]                          == INEEDMD_CMND_SOF) & \
             (tRadio_msg.uiBuff[tRadio_msg.uiBuff_size - 1] == INEEDMD_CMND_EOF))
          {
            eIneedmd_cmnd_Proto_ReqNote_params_init(&tProtocol_req_notify);
            tProtocol_req_notify.eReq_Notify = CMND_REQUEST_RCV_PROTOCOL_FRAME;
            tProtocol_req_notify.uiCmnd_Frame = tRadio_msg.uiBuff;
            tProtocol_req_notify.uiFrame_Len = tRadio_msg.uiBuff_size;
            eIneedmd_cmnd_Proto_Request_Notify(&tProtocol_req_notify);
          }
          else
          {
