...
void vUSB_task(UArg arg0, UArg arg1)
{
  ...
  vDEBUG_ASSERT("vUSB_task, Can't create semaphore", semUSB_task != NULL);

  eEC = eUSB_mass_storage_setup();
  vDEBUG_ASSERT("vUSB_task, USB setup failure", (eEC == ER_OK));

  eEC = eUSB_MSCD_check_physical_connection();
  if(eEC == ER_CONNECTED)
  {
    while(eUSB_notify_callbacks(true, false) == ER_FAIL)
    {
      Task_sleep(500);
    }
  }
